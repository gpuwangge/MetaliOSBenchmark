...
func mainFunc() {
  ...
  // Register file overestimate:   2x28x32 threads, 224 bytes = 392 KB
    ...
    let numTransforms = 2//20
    let baseNumThreads = 4;//32
    let tgmemPerSimdgroup = 256;//1024
  let simdsPerThreadgroup = 8//28
  ...
  do {
    ...
    for transformIndex in 0..<numTransforms {
      ...
      constants.setConstantValue(&_transformIndex, type: .int, index: 1)
        ...
        let function_name = "testThroughput\((transformIndex % 32) + 1)"
        //CContext.global.CreatePipeline(functionName: function_name)
        //pipelines.append(CContext.global.pipelines[function_name]!)
    
        let function = try! CContext.global.library
        .makeFunction(name: function_name, constantValues: constants)
        let pipeline = try! CContext.global.device
      ...
      if let maxThreadgroupSize = maxThreadgroupSize {
        ...
        //maxThreadgroupSize = CContext.global.pipelines[function_name]!.maxTotalThreadsPerThreadgroup
...
//    let heap = device.makeHeap(descriptor: heapDesc)!
    ...
    for _ in 0..<numTransforms {
        let buffer = CContext.global.device.makeBuffer(
  ...
  for _ in 0..<5 {
    ...
    for commandIndex in 0..<numCommands {
      ...
      //ERROR: failed assertion `Dispatch Threads with Non-Uniform Threadgroup Size is not supported on this device'
