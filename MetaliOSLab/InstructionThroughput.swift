...
//      1 : (32, 32 * 1024), // 11 (30 simds)
      2 : (23, 16 * 1024), // 19 (46 simds) original: 31
      3 : (15, 16 * 1024), // 28 (45 simds) original: 20
...
//      7 : (10,  8 * 1024), // 67 (70 simds)
  ...
  //let pipeline = try! device.makeComputePipelineState(function: function)
    ...
    let function_name = "testCache"
    CContext.global.CreatePipeline(functionName: function_name)
    
  let maxTotalThreadsPerThreadgroup = CContext.global.pipelines[function_name]!.maxTotalThreadsPerThreadgroup
    
  let maxSimdsPerThreadgroup = maxTotalThreadsPerThreadgroup / 32
  if maxSimdsPerThreadgroup < 32 {
    //print("Overallocated registers: \(maxSimdsPerThreadgroup) simds \ maxTotalThreadsPerThreadgroup")
    print("maxTotalThreadsPerThreadgroup: ", maxTotalThreadsPerThreadgroup)
    print("maxSimdsPerThreadgroup: ", maxSimdsPerThreadgroup)
    print("Overallocated registers")
  }
  
    //Step 5: create input/ouput buffer
  //let inputsBuffer = device.makeBuffer(length: 1024 * 1024)!
  //let outputsBuffer = device.makeBuffer(length: 1024 * 1024)!
  let inputsBuffer = CContext.global.device.makeBuffer(length: 1024 * 1024)!
  let outputsBuffer = CContext.global.device.makeBuffer(length: 1024 * 1024)!
  ...
  for _ in 0..<numTrials {
    for residentThreadgroups in residentThreadgroupsRange {
      ...
      defer {
        ...
        //Step 6: set encoder and commit command buffer
